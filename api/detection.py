...
def detect_fire():
    ...
    try:
        ...
        if "file" not in request.files:
            return jsonify({"success": False, "error": "未找到上傳檔案"}), HTTP_STATUS["BAD_REQUEST"]

        file = request.files["file"]
        if file.filename == "":
            return jsonify({"success": False, "error": "未選擇檔案"}), HTTP_STATUS["BAD_REQUEST"]

        # 獲取參數並進行輸入驗證
        role = validate_role(request.form.get("role", ""))
        use_ai = validate_boolean(request.form.get("use_ai", "false"))
        ai_provider = validate_ai_provider(request.form.get("ai_provider", "openai"))
        ...
        if use_ai and ai_provider not in AI_PROVIDERS:
            return (
                jsonify({"success": False, "error": f"無效的AI提供者，支援: {list(AI_PROVIDERS.keys())}"}),
                HTTP_STATUS["BAD_REQUEST"],
            )
        ...
        if not upload_success:
            return jsonify({"success": False, "error": error_msg}), HTTP_STATUS["BAD_REQUEST"]

        # 進行火災檢測
        prediction_result = model_manager.predict_fire(file_path)
        if prediction_result is None:
            return (
                jsonify({"success": False, "error": "模型預測失敗，請檢查模型是否正確載入"}),
        ...
        return jsonify({"success": True, "data": response_data}), HTTP_STATUS["OK"]

    except Exception as e:
        logger.error(f"火災檢測API發生錯誤: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "系統內部錯誤，請稍後再試"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )


def _get_ai_analysis(
    image_path: str, role: str, is_fire: bool, fire_probability: float, ai_provider: str
) -> str:
    """
    獲取AI分析結果
    ...
    try:
        # 構建提示詞
        role_name = sanitize_string_input(USER_ROLES.get(role, "使用者"), max_length=50)
...
請提供實用、具體的專業建議，避免一般性說明。"""
        ...
        logger.error(f"AI分析失敗: {str(e)[:200]}")
        return ""
...
def get_detection_status():
    ...
    try:
        ...
        logger.error(f"獲取系統狀態失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "無法獲取系統狀態"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )
