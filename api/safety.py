...
def get_fire_safety_advice():
    ...
    try:
        ...
        logger.error(f"獲取安全建議時發生錯誤: {str(e)[:200]}")
        return jsonify({"error": "獲取安全建議失敗，請稍後再試"}), HTTP_STATUS["INTERNAL_SERVER_ERROR"]
...
def get_general_safety_tips():
    ...
    try:
        ...
        return jsonify({"success": True, "data": tips}), HTTP_STATUS["OK"]

    except Exception as e:
        logger.error(f"獲取一般安全建議失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "獲取安全建議失敗"}),
...
def get_situation_advice(situation):
    ...
    try:
        ...
        if not advice:
            return (
                jsonify({"success": False, "error": f'未找到情況 "{situation}" 的建議'}),
                HTTP_STATUS["BAD_REQUEST"],
            )

        return (
            jsonify({"success": True, "situation": situation, "advice": advice}),
            HTTP_STATUS["OK"],
        )

    except Exception as e:
        logger.error(f"獲取情況建議失敗: {str(e)[:200]}")
        return jsonify({"success": False, "error": "獲取建議失敗"}), HTTP_STATUS["INTERNAL_SERVER_ERROR"]
...
def get_emergency_contacts():
    ...
    try:
        ...
        return jsonify({"success": True, "data": contacts}), HTTP_STATUS["OK"]

    except Exception as e:
        logger.error(f"獲取緊急聯絡方式失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "獲取緊急聯絡方式失敗"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )
...
def get_safety_checklist():
    ...
    """
    try:
        environment = sanitize_string_input(
            request.args.get("environment", "general"), max_length=30, default="general"
        )

        checklist = safety_service.get_safety_checklist(environment)

        return (
            jsonify({"success": True, "environment": environment, "checklist": checklist}),
            HTTP_STATUS["OK"],
        )

    except Exception as e:
        logger.error(f"獲取安全檢查清單失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "獲取檢查清單失敗"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )
...
def get_role_based_advice():
    ...
    try:
        if not request.is_json:
            return jsonify({"success": False, "error": "請求必須為JSON格式"}), HTTP_STATUS["BAD_REQUEST"]

        data = request.json
        role = validate_role(data.get("role"))
        is_fire = validate_boolean(data.get("is_fire", False))
        fire_probability = validate_probability(data.get("fire_probability", 0.0))

        # 驗證角色 (已由 validate_role 處理)
        if role not in USER_ROLES:
            return (
                jsonify({"success": False, "error": f"無效的角色，支援的角色: {list(USER_ROLES.keys())}"}),
                HTTP_STATUS["BAD_REQUEST"],
            )

        # 生成個性化建議
        advice = safety_service.generate_role_based_advice(role, is_fire, fire_probability)

        return jsonify({"success": True, "data": advice}), HTTP_STATUS["OK"]

    except Exception as e:
        logger.error(f"獲取角色建議失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "獲取個性化廚議失敗"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )
...
def validate_sop_data():
    ...
    try:
        ...
        return jsonify({"success": True, "data": validation_result}), HTTP_STATUS["OK"]

    except Exception as e:
        logger.error(f"SOP數據驗證失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "SOP數據驗證失敗"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )
...
def get_available_roles():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "data": {"roles": roles, "total": len(roles)}}),
            HTTP_STATUS["OK"],
        )

    except Exception as e:
        logger.error(f"獲取角色列表失敗: {str(e)[:200]}")
        return (
            jsonify({"success": False, "error": "獲取角色列表失敗"}),
            HTTP_STATUS["INTERNAL_SERVER_ERROR"],
        )
